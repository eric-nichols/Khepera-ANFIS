...
LEFT = .2;    % .2==left
RIGHT = -.2;  % -.2==right
greaterThan = true; % initialise boolean greaterThan
...
for i=1:300
  ...
  if (anfisOutput > LEFT) % The object is Left
      turn(LEFT, 5, -5); % turn right
  else if (anfisOutput < RIGHT) % The object is right
      turn(RIGHT, -5, 5); % turn left
  else % forward
      kSetSpeed(ref,-5,-5);
  end
  end%end if 
...
function [ ] = turn( direction, lWheel, rWheel )
  global anfisOutput ref out_fis LEFT RIGHT; % global variables
  kSetSpeed(ref, lWheel, rWheel); % move in the parametered direction
...
function [ ] = trainingMatrix( darker )
  ...
  positive = [positive; 1 1024 1];%add the maximum possible value
  ...
  zero = [zero1; zero2];% combine the matrixes
  trainingData = [positive; negative; zero];%concact training data
end